...
fn view(app: &App, frame: Frame) {
    ...
    let rect = Rect::from_w_h(100.0, 100.0).top_left_of(win_p);
    draw.rect().xy(rect.xy()).wh(rect.wh()).color(PLUM);

    // rect-aligned ellipsis
    let ellip_to_rect = rect.below(rect);
    draw.ellipse()
        .xy(ellip_to_rect.xy())
        .wh(ellip_to_rect.wh())
        .color(RED);
